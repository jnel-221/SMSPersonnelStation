...
async function switchRole(deptid) {
  switch (deptid) {
    ...
    case "Administration":
      ...
      case "NICU":
        let sql6 = `SELECT empid, first_name, last_name, title, salary, dept_name FROM employee LEFT JOIN role ON employee.roleid = role.roleid LEFT JOIN department ON role.deptid = department.deptid WHERE dept_name = 'NICU'`;
  
        connection.query(sql6, function (err, res) {
          if (err) throw err;
